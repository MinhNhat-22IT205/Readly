...
const login = async (values: ztLoginInputs): Promise<LoginResponse | ServerError> => {
  try {
    ...
    return (error as any).response?.data ?? { message: "Unknown error", statusCode: 500, error: "Unknown" };
  }
};
